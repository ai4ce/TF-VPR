...
def validate(temp_index):
    ...
    nbrs_100 = NearestNeighbors(n_neighbors=100, algorithm='ball_tree').fit(best_matrix_sec)
        
    distance_100, indices_100 = nbrs_100.kneighbors(best_matrix_sec)
    np.save("supervised.npy", indices_100)
    indices_100 = np.load("supervised.npy")
    ...
    for i in range(len(indices_gt)):
        ...
        indice_retrieve_per = list(indices_100[i])
        if len(indice_gt_per) > 0 :
            ...
            for pos_ind in pos_index_range:
                try:
                    ...
                    pass
                try:
                    indice_retrieve_per.remove(i+pos_ind)
                except:
                    pass
            '''
            ...
            '''
        # print(indice_gt_per)
        # print(indice_retrieve_per)
        # assert()
        indice_10_per = indice_retrieve_per[:10]
        indice_5_per = indice_retrieve_per[:5]
        indice_1_per = indice_retrieve_per[:1]
