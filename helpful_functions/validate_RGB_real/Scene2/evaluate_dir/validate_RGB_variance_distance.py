...
def validate(temp_index):
    ...
    #mode = "Our_method_RGB"
    mode = "Our_method_RGB_distance"
    #mode = "Our_method_RGB_ongoing"
   ...
   # mode = "Method_2_RI_RGB_no_feat__w_temporal"
    ...
    elif mode == "Our_method_RGB_ongoing":
        new_dir = "/home/cc/Our_method_RGB_ongoing/results/"
    elif mode == "Our_method_RGB_distance":
        new_dir = "/mnt/NAS/home/xinhao/loop_closure/Our_method_RGB_w_distance_check/results/"    
    elif mode == "Method_2_RI_RGB_no_feat__w_temporal":
        new_dir = "/home/cc/Method_2_RI_RGB_no_feat__w_temporal/results/Micanopy"
    save_name = os.path.join(new_dir,"scene2",'database'+str(temp_index)+'.npy')
    best_matrix = np.load(save_name)
    best_matrix = torch.tensor(best_matrix, dtype = torch.float64)
    best_matrix = np.array(best_matrix)
    
    data_dir = '/mnt/NAS/home/xinhao/loop_closure/dataset/scene2_calibrated_cc'
    ...
    nbrs_21 = NearestNeighbors(n_neighbors=21, algorithm='ball_tree').fit(best_matrix_sec)
        
    #distance_1, indices_1 = nbrs_1.kneighbors(best_matrix_sec)
    #distance_5, indices_5 = nbrs_5.kneighbors(best_matrix_sec)
    distance_21, indices_21 = nbrs_21.kneighbors(best_matrix_sec)
    indices_21_backup = []
    
    indices_10 = []
    indices_5 = []
    indices_1 = []
    for i in range(len(indices_21)):
        indices_21_per = indices_21[i]
        #print("indices_21_per_after:"+str(indices_21_per))
        #indices_21_backup.append(indices_21_per)
        indices_10.append(indices_21_per[:11])
        indices_5.append(indices_21_per[:6])
        indices_1.append(indices_21_per[:2])
        
    indices_10 = np.array(indices_10)
    indices_5 = np.array(indices_5)
    ...
    rolls = []
    for i in range(location_ests.shape[0]):
        '''
        w, x, y, z = ori_ests[i][0], ori_ests[i][1], ori_ests[i][2], ori_ests[i][3]
        roll = math.atan2(2*y*w - 2*x*z, 1 - 2*y*y - 2*z*z);
        '''
        if i ==0:
            roll = math.atan2(location_ests[i+1,1] - location_ests[i,1],location_ests[i+1,0] - location_ests[i,0])
        elif i==location_ests.shape[0]-1:
            roll = math.atan2(location_ests[i,1] - location_ests[i-1,1],location_ests[i,0] - location_ests[i-1,0])
        else:
            roll = math.atan2(location_ests[i+1,1] - location_ests[i-1,1],location_ests[i+1,0] - location_ests[i-1,0])
        rolls.append(roll)
    ...
    #8321, 1, 3, 4, 4965, 4964, 4963, 14440, 364, 3932
